...
pub fn build(b: *std.Build) void {
    ...
    const common_flags = &[_][]const u8{
        "-D_XOPEN_SOURCE", "-D_DARWIN_C_SOURCE", "-D_DARWIN_UNLIMITED_SELECT", "-D_REENTRANT", "-std=gnu11", "-fcommon", "-DHAVE_CONFIG_H",
    ...
    // Handle coroutine
    const coroutine_impl = switch (target.result.cpu.arch) {
        .aarch64 => "arm64",
        .x86_64 => "amd64",
        else => "copy",
    };
    
    if (std.mem.eql(u8, coroutine_impl, "arm64")) {
